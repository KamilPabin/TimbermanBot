...
class Log:
    ...
    def __init__(self, screen_handle: MSSBase, position: int):
        ...
        self.__left_branch_bounding_box = {
            "top": int(SystemProperties.screen_height - SystemProperties.tree_offset - position * Log.height + Log.skip_top_pixels),
            "left": int(SystemProperties.screen_width / 2 - Log.width / 2 - Log.branch_width),
            "width": int(Log.branch_width - Log.offset_from_log),
            "height": int(Log.height - Log.skip_top_pixels)
        }
        self.__right_branch_bounding_box = {
            "top": int(SystemProperties.screen_height - SystemProperties.tree_offset - position * Log.height + Log.skip_top_pixels),
            "left": int(SystemProperties.screen_width / 2 + Log.width / 2 + Log.offset_from_log),
            "width": int(Log.branch_width - Log.offset_from_log),
            "height": int(Log.height - Log.skip_top_pixels)
        }
